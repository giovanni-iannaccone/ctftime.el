...
The plugin can be customized through `M-x customize-group RET ctftime`.
...
The main buffer provides a simple `tabulated-list` interface.
...
Filters can be combined, so you can for example search for `pwn` while displaying only online Jeopardy CTFs.
